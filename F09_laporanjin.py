...
def laporanjin():
    ...
    list_jin_pembangun = ["%" for _ in range(len(username_arr))]

    for i in range(2, len(role_arr)):
        if role_arr[i] == "jin_pengumpul":
            total_jin_pengumpul += 1
        elif role_arr[i] == "jin_pembangun":
            total_jin_pembangun += 1
            list_jin_pembangun[i] = username_arr[i]

    total_jin = total_jin_pengumpul + total_jin_pembangun
    data_jin_pembangun = [["%", 0] for _ in range(total_jin_pembangun)]
    
    for i in range(len(username_arr)):
        if list_jin_pembangun[i] == "%":
            continue
        ...
        for i in range(total_jin_pembangun):
            ...
            if data_jin_pembangun[i][1] == max_rajin:
                if data_jin_pembangun[i][0] < jin_terajin:
                    jin_terajin = data_jin_pembangun[i][0]
            if data_jin_pembangun[i][1] < min_malas and data_jin_pembangun[i][0] in [row[1] for row in data_candi if len(row) >= 2]:
